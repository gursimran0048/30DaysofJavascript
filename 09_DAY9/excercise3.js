...
countriesData.sort((a, b) => {
  ...
  function findMostSpokenLanguages(data) {
    const languageCounts = data.reduce((counts, country) => {
    ...
    const topTenLanguages = sortedLanguages.slice(0, 10).map(([language, count]) => ({ language, count }));
    return topTenLanguages;
  }
  
  const mostSpokenLanguages = findMostSpokenLanguages(countriesData);
  console.log(mostSpokenLanguages);


  //** Use countries_data.js file create a function which create the ten most populated countries

  function findMostPopulatedCountries(data,no) {
    data.sort((a, b) => b.population - a.population);
    return data.slice(0, no);
  }
  
  const mostPopulatedCountries = findMostPopulatedCountries(countriesData,3);
  ...
  const statistics = {
    ...
    mode: function () {
      ...
      ages.forEach(age => {
        ...
        if (countMap.get(age) > maxCount) {
          maxCount = countMap.get(age);
          mode = age;
        }
      });
    ...
    var: function () {
      ...
      return squaredDifferences.reduce((total, squaredDiff) => total + squaredDiff, 0) / ages.length;
    },
    ...
    freqDist: function () {
      ...
      return Array.from(countMap.entries()).map(([age, count]) => [count / ages.length * 100, age]);
    },
    describe: function () {
        return {
          count: this.count(),
